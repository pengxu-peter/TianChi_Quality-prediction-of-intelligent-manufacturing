...
v5相比的改进：
1. 剔除和Y相关度很小的数据(Train的方差小于TestA和TestB的)；
...
class DataPreprocess:
    ...
    def nan_replace(self, mode = 'mean'):
        ...
        #提取可能要删除的index            
        self.nan_ignore_index = list(list(np.where(self.nan_num_use > 20))[0])      #阈值可以选择100（200个特征）、20（1684个特征）
    ...
    def correlation_get(self):
        ...
        correlation_threshold = 0.02   #当前阈值设置为0.02, 0.05
        for i in range(col):
    ...
    def train_test_diff(self, diff_threshold=[1, 0.5, 0.2, 0.1, 0.05]):
        ...
        train_std = np.std(self.new_array_data[0:train_len,:],axis=0)
        testA_std = np.std(self.new_array_data[train_len:(train_len+testA_len),:],axis=0)
        testB_std = np.std(self.new_array_data[(train_len+testA_len):(train_len+testA_len+testB_len),:],axis=0)
        train_testA_std_diff = testA_std - train_std #差值越大，说明这个特征不能使用
        train_testB_std_diff = testB_std - train_std
        ...
        #将train_testA_std_diff差别很大的也直接剔除出去
        std_diff_threshold = 0.1
        self.std_diff_index = []
        for i in range(len(train_testA_std_diff)):
            if (train_testA_std_diff[i]>std_diff_threshold) or (train_testB_std_diff[i]>std_diff_threshold):
                self.std_diff_index.append(i)
        logging.info('std_diff_cut successfully !!!, cut number is %d' %(len(self.std_diff_index)))
    ...
    def data_seperate_transfer_save(self):
        ...
        for diff_threshold_item in self.diff_threshold:
            ...
            del_index_3 = list(set(del_index_2).union(set(self.correlation_index))) #避免重复，其实也不会有重复
            del_index = list(set(del_index_3).union(set(self.std_diff_index))) #避免重复，其实也不会有重复
...
def main():
    ...
    dp.correlation_get()
    dp.normalization()
    dp.train_test_diff(diff_threshold=[0.8, 0.6, 0.4]) #
