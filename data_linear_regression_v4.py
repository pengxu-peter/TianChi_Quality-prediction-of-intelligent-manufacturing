...
class DataLinReg:
    def __init__(self):
        ...
        self.data_directory = r'temp_data_v6_v3\(0.02_20_0.2)\mean_diff_0.4_pca_99'
    ...
    def data_read(self):
        ...
        #将train的数据进行分割，用于训练
        model = build_model(self.new_data['训练'], self.new_data['Y'])
        print('predict and submit...')
        subA = model.predict(self.new_data['测试B'])
        # read submit data
        sub_df = pd.read_csv('测试B-答案模板.csv',header=None)
        sub_df['Y'] = subA
        sub_df.to_csv('github_B.csv',header=None,index=False)        
