...
class DataPreprocess:
    ...
    def almost_same_kick(self, ask_threshold=0.05):
        #将几乎都是相同的数据剔除出去，当前采用std/mean的方法进行判断，效果很差，不能使用
        (row, col) = self.new_array_data.shape
        self.ask_index = []
        self.ask_threshold = ask_threshold
        for i in range(col):
            check_value = abs(self.data_std[i]/self.data_mean[i])
            if check_value < ask_threshold: #算法过程有很多地方出现了累计误差（np.mean等会有小数点后多位之后出现误差），这里严格为0太少了，需要设置一个阈值，原始数据最小值为1e-10
                self.ask_index.append(i)
    ...
    def data_seperate_transfer_save(self):
        ...
        for diff_threshold_item in self.diff_threshold:
            ...
            del_index = list(set(self.diff_index[diff_threshold_item]).union(set(self.std_zero))) #避免重复，其实也不会有重复
